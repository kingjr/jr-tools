...
from cloud.utils import multi_client

assert_raises(ValueError, multi_client, 'foo', 'a')

for server, credentials in zip(['S3', 'Dropbox'], ['boto.cfg', 'dropbox.pem']):
    cred_file = op.join('./cloud/.credentials/', credentials)
    route = multi_client(server, cred_file, bucket='jrking.test')
    # check clean
    ...
    assert_equal(op.getsize('foo2.npy'), route.metadata('foo.npy')['bytes'])
    assert_true(not route.download('foo.npy', 'foo2.npy', overwrite=False))
    # skips if identical sizes?
    assert_true(not route.download('foo.npy', 'foo2.npy', overwrite='auto'))
